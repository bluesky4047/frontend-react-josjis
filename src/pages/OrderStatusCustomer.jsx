  ...
  if (!orderId) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 text-center">
        <h2 className="font-roboto font-bold text-3xl mb-4 text-[#FFD900]">
          Belum Ada Pesanan Aktif
        </h2>
        <p className="text-gray-400 mb-8 max-w-md">
          Kamu belum melakukan pemesanan makanan. Silakan pesan makanan terlebih
          dahulu di menu utama.
        </p>
        <button
    ...
    <div className="min-h-screen bg-black relative overflow-x-hidden font-roboto pt-[80px]">
      ...
      <div className="relative h-[373px] w-full overflow-hidden flex flex-col items-center justify-center text-center px-8">
        ...
        <div className="relative z-10 text-white space-y-6">
          <h2 className="font-roboto font-bold text-[48px] leading-tight drop-shadow-lg uppercase tracking-tight">
            Lacak Pesananmu
          </h2>
          <p className="text-[24px] font-normal leading-relaxed opacity-90 max-w-2xl">
            Pantau setiap langkah, dari dapur ke mejamu.
          </p>
        </div>
      ...
      <main className="relative z-10 py-20 px-8 md:px-[170px] flex flex-col items-center">
        ...
        <div className="w-full max-w-[1220px] bg-white/10 backdrop-blur-xl rounded-[25px] border border-white/20 p-12 mb-12 shadow-2xl">
          <h3 className="font-roboto font-bold text-[40px] text-white text-center mb-12">
            Ringkasan Pesanan
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="bg-black/20 border border-white/10 rounded-[12px] p-6 flex gap-6 items-center">
              ...
              <div className="space-y-2">
                <p className="text-[20px] font-medium text-white break-all">
                  Order #{orderId.slice(0, 8).toUpperCase()}
                </p>
                <span className="inline-block bg-[#FFD464]/50 border border-black/10 px-2 py-1 rounded-[4px] text-[12px] text-white">
                  Nomor Pesanan
                </span>
              </div>
            </div>
            ...
            <div className="bg-black/20 border border-white/10 rounded-[12px] p-6 flex gap-6 items-center">
              ...
              <div className="space-y-1">
                ...
                <span className="inline-block bg-[#FFD900]/50 border border-black/10 px-2 py-1 rounded-[4px] text-[12px] text-white">
                  {statusInfo.badge}
                </span>
              </div>
            </div>
            ...
            <div className="bg-black/20 border border-white/10 rounded-[12px] p-6 flex gap-6 items-center">
              ...
              <div className="space-y-1 text-white">
                <p className="text-[20px] font-medium">
                  {order?.tables?.name ? `Meja` : "Take Away"}
                </p>
                <p className="text-[16px] opacity-80">{order?.tables?.name}</p>
              </div>
            </div>
        ...
        <div className="w-full max-w-[1220px] bg-white/10 backdrop-blur-xl rounded-[25px] border border-white/20 p-12 mb-12 shadow-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {(() => {
              ...
              return (
                <div
                  className={`text-center space-y-2 border-r border-white/10 last:border-0 p-4 ${s.label === "Sekarang" ? "bg-white/5 rounded-xl" : ""} ${!s.active ? "opacity-40" : ""}`}
                >
                  <p className="text-white/60 uppercase tracking-widest text-sm font-bold">
                    Diterima
                  </p>
        ...
        <div className="w-full max-w-[1220px] bg-white/10 backdrop-blur-xl rounded-[25px] border border-white/20 p-12 mb-12 shadow-2xl">
          <div className="flex flex-col md:flex-row gap-12 items-center">
            <div className="flex-1 space-y-4">
              <h3 className="font-roboto font-bold text-[40px] text-white">
                Progres Status
              </h3>
        ...
        <div className="w-full max-w-[1220px] bg-white/10 backdrop-blur-xl rounded-[25px] border border-white/20 p-12 shadow-2xl">
          <h3 className="font-roboto font-bold text-[40px] text-white mb-12">
            Item yang Dipesan
          </h3>
          ...
          <div className="space-y-6">
            {order?.order_items?.map((item, idx) => (
              <div
                key={item.id || idx}
                className="flex justify-between items-center py-6 border-b border-white/10 last:border-0 group hover:bg-white/5 transition-colors px-4 rounded-lg"
              >
                <div className="space-y-1">
                  <p className="text-[24px] font-medium text-white">
                    {item.products?.name}
                  </p>
                  <p className="text-[18px] text-white/60 italic font-light">
                    Jumlah {item.qty}
                  </p>
                </div>
